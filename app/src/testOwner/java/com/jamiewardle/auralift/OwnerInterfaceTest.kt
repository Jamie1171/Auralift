...
class OwnerInterfaceTest {
    ...
    @Test fun nativeSettingsThemesHelpAndPoliciesAreReachable() {
        ...
        screenshot("help")
        compose.onNodeWithText("Feedback & support").performScrollTo().performClick()
        compose.onNodeWithText("Your message").performTextInput("Testing feedback locally.")
        compose.onNodeWithText("Open email draft").performScrollTo().assertIsEnabled()
        screenshot("feedback")
        compose.onNodeWithText("Settings", useUnmergedTree = true).performClick()
